...
**New endpoints (replaces `POST /api/templates/{id}/generate`):**
...
### Requirement: `POST /api/templates/{id}/generate`
**Reason:** Replaced by `POST /api/templates/{id}/jobs` which accepts an array of records and creates a job container.
**Migration:** Callers pass `{ "records": [payload] }` (array with one element) to `POST /api/templates/{id}/jobs`, then `POST /api/jobs/{id}/submit` to trigger rendering. The single-record flow is a degenerate case of the multi-record job.

### Requirement: `GET /api/jobs/{id}` returns single-job status
**Reason:** The `GET /api/jobs/{id}` endpoint is redefined to return job container status including item list. The response shape changes.
**Migration:** The `download_url` field is removed from the job response; download URLs are per-item at `GET /api/jobs/{id}/items/{item_id}/download`. Callers must update to the new response shape.

### Requirement: `GET /api/jobs/{id}/download` streams a single PDF
**Reason:** Redefined to stream a ZIP of all completed items. Single-item download is now at `GET /api/jobs/{id}/items/{item_id}/download`.
**Migration:** Update callers to use the item-level download endpoint for single-document downloads.
