...
import { fieldIntentNodeViewRenderer } from "../editor/FieldIntentNodeView";
...
function buildBrandCSS(s: StylesheetDef): string {
  ...
  if (s.headingColour)
    vars.push(`  --brand-heading-colour: ${s.headingColour}`);
  ...
  for (const n of HEADING_LEVELS) {
    ...
    if (style?.spacingBefore !== undefined)
      vars.push(`  --brand-h${n}-before: ${style.spacingBefore}pt`);
    if (style?.spacingAfter !== undefined)
      vars.push(`  --brand-h${n}-after: ${style.spacingAfter}pt`);
  }
  if (s.normal?.fontSize !== undefined)
    vars.push(`  --brand-normal-size: ${s.normal.fontSize}pt`);
  if (s.normal?.spacingBefore !== undefined)
    vars.push(`  --brand-normal-before: ${s.normal.spacingBefore}pt`);
  if (s.normal?.spacingAfter !== undefined)
    vars.push(`  --brand-normal-after: ${s.normal.spacingAfter}pt`);
  ...
  for (const n of HEADING_LEVELS) {
    ...
    if (style?.fontSize !== undefined)
      decls.push(`font-size: var(--brand-h${n}-size)`);
    if (style?.spacingBefore !== undefined)
      decls.push(`margin-top: var(--brand-h${n}-before)`);
    if (style?.spacingAfter !== undefined)
      decls.push(`margin-bottom: var(--brand-h${n}-after)`);
    if (decls.length) {
      rules.push(
        `.brand-editor .ProseMirror h${n} {\n  ${decls.join(" !important;\n  ")} !important;\n}`,
      );
    }
  }
  ...
  const bDecls: string[] = [];
  if (s.bodyFont)
    bDecls.push(`font-family: var(--brand-body-font), sans-serif`);
  if (s.bodyColour) bDecls.push(`color: var(--brand-body-colour)`);
  if (s.normal?.fontSize !== undefined)
    bDecls.push(`font-size: var(--brand-normal-size)`);
  if (s.normal?.spacingBefore !== undefined)
    bDecls.push(`margin-top: var(--brand-normal-before)`);
  if (s.normal?.spacingAfter !== undefined)
    bDecls.push(`margin-bottom: var(--brand-normal-after)`);
  if (bDecls.length) {
    rules.push(
...
export default function BlockCanvas({
  ...
  const [intentPopover, setIntentPopover] = useState<IntentPopoverState | null>(
    null,
  );
  ...
  const editor = useEditor({
    extensions: [
      ...
      TableHeader,
      TableCell,
      FieldIntentNode.configure({
        nodeViewRenderer: fieldIntentNodeViewRenderer,
        templateId: templateId ?? "",
        onSwitchToData: onSwitchToData ?? null,
      }),
      SectionNode.configure({ nodeViewRenderer: sectionNodeViewRenderer }),
    ],
    content: ptToProsemirror(blocks),
    onUpdate: ({ editor }) => {
      const pmJson = editor.getJSON() as {
        content?: Parameters<typeof prosemirrorToPt>[0]["content"];
      };
    ...
    editorProps: {
      attributes: {
        class:
          "outline-none min-h-[60vh] prose max-w-none prose-zinc focus:outline-none",
      },
